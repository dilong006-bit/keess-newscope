# KEESS 인재키움 프리미엄 — '공개교육' 탭 신설 기술명세서 v1.0
...
| OP-07 | 공개교육 전용 FAQ **2문항** (원문 고정) | 요청 [3] |
...
| `components/kium/KiumOpenFaq.tsx` | 공개교육 FAQ 2문항 |
...
| `lib/kium/content.ts` | `open` 블록 추가 (문안·FAQ·캡션) | 낮음 |
...
3. 딥링크 처리(§7-4)
4. `KiumOpenHero` / `KiumSchedule` / 과정 소개(`KiumCourseGrid` 재사용) / `KiumOpenFaq` 조립
...
[작업 3] 공개교육 탭 컴포넌트
  ...
  KiumSessionStatus / KiumOpenFaq / KiumApplySummary 를 §5·§6·§9 명세대로 생성.
...
**18번 사유**: 요청 원문 표기 `12/17(수)~18(금`이 실제 달력(2026-12-17=목요일)과 불일치. 해당 과정의 10·11월 패턴은 일관되게 수~목. 사업부 회신으로 `12/16(수)~17(목)` 또는 `12/17(목)~18(금)` 확정 필요.
...
| # | 항목 | 명세상 처리 | 회신 후 조치 |
| --- | --- | --- | --- |
| C1 | Re-Lead 12월 회차 일자 | `relead-r3`를 `tbd: true`로 두고 `일정 조율 중` 렌더 | `sessions.ts` 1줄 수정 |
| C2 | 「공개교육 일자」 원본 파일 | 부록 A는 PDF 판독 + 달력 역산본 | 원본 대조 후 확정 |
